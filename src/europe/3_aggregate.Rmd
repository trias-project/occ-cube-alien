...
taxa_species_eu <- 
  ...
  map(~select(.x, speciesKey, scientificName, rank, taxonomicStatus, kingdom)) %>%
...
Accepted infraspecific taxa in the modelling list:
...
Some taxa in the modelling list are synonyms. For these taxa we don't trust the link to accepted taxa provided by GBIF.

Get synonyms from the list:
...
alien_taxa_synonyms_key <- modelling_taxa_synonyms_key[
  which(modelling_taxa_synonyms_key %in% 
          unique(occ_cube_eu_synonym$taxonKey))]
paste(length(alien_taxa_synonyms_key),
  "out of",
...
For these taxa we don't have occurrences linked to other related taxa. We have just to retrieve scientific name, taxonomic status and rank. We extract this information and make a data.frame called `taxa_synonym_eu`. In this case the column `includes` is trivial:

```{r taxa_synonym_eu}
taxa_synonym_eu <- if (length(alien_taxa_synonyms_key) > 0) {
