...
modelling_taxa <- read_tsv(
  here::here("data", "raw", "modelling_species.tsv"),
  na = "",
  quote = "",
  col_types = cols(
...
```{r define_col_integer_for_specific_columns}
int_colnames <- 
...
```{r define_col_double_for_specific_columns}
...
```{r define_col_text_for_other_columns}
...
occ_eu <- read_tsv(
  here::here("data", 
             "interim", 
             paste0("0004690-190415153152247","_occurrence.tsv")),
  na = "",
  quote = "",
  col_types = col_specs)
```
...
```{r modelling_taxa_species_accepted}
...
```{r modelling_taxa_under_species}
...
```{r get_modelling_taxa_subspecies_key}
...
```{r get_modelling_taxa_synonyms_key}
...
Merge the three data.frames in a single *cube*, `occ_cube_europe`:

```{r make_occ_cube_europe}
occ_cube_europe <- 
  occ_cube_eu_species %>%
...
Merge the three data.frames in a single data.frame, `taxa_europe`:
...
```{r save_datacube_europe}
write_tsv(occ_cube_europe, 
          here::here("data", "processed", "cube_europe.tsv"),
          na = ""
)
```
...
```{r save_taxa_europe}
write_tsv(taxa_europe, 
          here::here("data", "processed", "cube_europe_taxa.tsv"),
          na = ""
)
```
