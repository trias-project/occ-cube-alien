...
## Aggregate data for accepted species
...
Number of infraspecific taxa not included in `occ_cube_subspecies`:
...
We create `taxa_subspecies` by adding the taxonomic rank, one of `SUBSPECIFICAGGREGATE`, `SUBSPECIES`, `VARIETY`, `SUBVARIETY`, `FORM`, or `SUBFORM`, and the taxonomic status, one of `ACCEPTED` or  `DOUBTFUL`, and create a column called `include` which contains all taxa whose occurrences are linked to the `acceptedTaxonKey`:
...
For these taxa we don't have occcurrences linked to other related taxa. We have just to retrieve scientific name, taxonomic status and rank of these taxa. We extract this information and make a data.frame called `taxa_synonym`. In this case the column `includes` is trivial:
...
# Save aggregated data and mapped taxa for Belgium
...
Merge the three data.frames in a single data.frame, `occ_cube_belgium_taxa`:
...
```{r save_baseline_datacube}
write_tsv(taxa, 
          here("data", "processed", "cube_belgium_taxa.tsv"),
          na = ""
)
```
