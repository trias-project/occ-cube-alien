...
occ_cube_baseline <- 
  ...
  mutate(kingdomKey = as.integer(kingdomKey))
```
...
## Select accepted species from checklist
...
For speeding up the search in database, we create an index on  `year`,  `eea_cell_code`, `speciesKey` if not present:
...
if (!idx_species_year_cell %in% indexes_all$name) {
  ...
  table_name = table_name,
  cols_idx = c("year", "eea_cell_code", "speciesKey"),
  .con = sqlite_occ
  )
  dbExecute(sqlite_occ, query)
}
...
Group by  `year`, `eea_cell_code` and `speciesKey` and count number of occurrences for each group:

```{r get_datacube_kingdom}
query <- glue_sql(
  "SELECT {`cols`*}, COUNT(_ROWID_) 
FROM {table}
GROUP BY {`cols`*}",
  cols = c("year", "eea_cell_code", "speciesKey"),
  table = table_name,
  .con = sqlite_occ
)
occ_cube_species <- 
