...
## Aggregate occurrences for accepted alien species and genera

### Aggregate occurrences

As initial step, we start by selecting occurrences linked to taxa with rank `SPECIES` or `GENUS` and taxonomic status `ACCEPTED` and `DOUBTFUL`:
...
The `nubKey` of these taxa are the `speciesKey` or `genusKey` we can filter on:
...
For speeding up the aggregation, we create an index on  `year`,  `eea_cell_code`, `speciesKey` and `genusKey` if not present:

```{r create_idx_speciesKey}
idx_species_year_cell <- "idx_species_genus_year_cell"
# get indexes on table
query <- glue_sql(
    "PRAGMA index_list({table_name})",
    table_name = table_name,
...
Group by `year`, `eea_cell_code`, `speciesKey`, `genusKey`, count number of occurrences for each group and get the lowest value of coordinate uncertainty `occurrenceUncertaintyInMeters`:
...
Remove occurrences at genus level, if present:

```{r remove_genus_level_occs}
occ_cube_species <-
  occ_cube_species %>% 
  filter(!is.na(speciesKey) & speciesKey != 0)
```

Number of alien species (or genera) included in the occurrence cube:
...
write_csv(occ_cube_baseline,
          here::here("data", "processed", "be_classes_cube.tsv"),
          na = "")
```
