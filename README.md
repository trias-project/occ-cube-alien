...
By aggregating we would loose this information, so we provide aside the cubes, `cube_belgium.tsv` and `cube_europe.tsv`, a kind of taxonomic compendium, [`cube_belgium_taxa.tsv`](https://github.com/trias-project/occ-processing/blob/master/data/processed/cube_belgium_taxa.tsv) and [`cube_europe_taxa.tsv`](https://github.com/trias-project/occ-processing/blob/master/data/processed/cube_europe_taxa.tsv) respectively. They include for each taxa in the cube all the synonyms or infraspecies whose occurrences contribute to the total count. They are both saved in `data/processed`.
